...
const refRowVaalidId = React.useRef<GridRowId | null>(null);
  ...
  const handleEditUser = async (newRow: GridRowModel) => {
    ...
    try {
        ...
        const response = await axiosInstance.patch<FuenteDB>(`v1/fuentes/${newRow.id}`, {...newRow});
        const fuenteUpdated = response.data;
        setfuentes(fuentes.map((fuente) => (fuente.id === newRow.id ? fuenteUpdated : fuente)));
  ...
  React.useEffect(() => {
    ...
    axiosInstance.get<FuenteDB[]>('v1/fuentes/get-all-fuentes').then(resp => {
  ...
  const handleSaveClick = (id: GridRowId) => () => {
    refRowValidModel.current = fuentes.find(fuente => fuente.id === id) || null;
    setopenDialog(true);
  };
  ...
  const columns: GridColDef[] = [
    ...
    { editable: true, field: 'editores', headerName: 'Editores', width: 130 },
    { editable: true, field: 'frequency', headerName: 'Frecuencia', width: 130 },
    { editable: true, field: 'isOpen', headerName: 'Abierta', type: 'string', width: 90 },
    { editable: true, field: 'materia', headerName: 'Materia', sortable: false, width: 160},
    { editable: true, field: 'organization', headerName: 'Organización', type: 'singleSelect', sortable: false, width: 160},
    { editable: true, field: 'title', headerName: 'Título', sortable: false, width: 160},
    { editable: true, field: 'url', headerName: 'Url', sortable: false, width: 160},
    {
        field: 'actions',
        type: 'actions',
        headerName: 'Actions',
  ...
  return (
    ...
    >
      ...
      />
      <AlertDialogSlide message = { 'Desea actualizar la fuente' } open = {openDialog} handleClose={handleClosedialog} handleAction={processRowUpdate} row={refRowValidModel.current!}/>
      <AlertDialogSlide message = { 'Desea Eliminar la fuente' } open = {openDialogDeleted} handleClose={handleCloseDialogDeleted} handleDelete={handleRemoveById} id={refRowVaalidId.current!}/>
      <CustomizedSnackbars 
